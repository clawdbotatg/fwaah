...
- **Deposit an NFT** — pick a whitelisted collection and pick one of your
  tokens from a select. No indexer: `tokensOfOwner` (one call) →
  `tokenOfOwnerByIndex` (batched) → Transfer-log scan verified with
  `ownerOf` (full history on hosted RPCs, the retained window on pruned
  home nodes). Ownership, whitelist and approval are re-checked with
  cheap `eth_call`s at deposit time; then approve → `listNFT` with the
  backing as `msg.value`. An OpenSea link sits next to the form for
  floor-checking before you choose a backing.
